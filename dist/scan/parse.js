...
const DEV_RE = /\b(node|deno|bun|next|vite|nuxt|npm|pnpm|yarn|webpack|esbuild|turbo|turbopack|remix|astro|gatsby|nodemon|ts-node|tsx|rollup|parcel|storybook|wrangler|rsbuild|rspack|expo|metro|sst)\b/i;
/** Decide whether a process looks like a dev server / JS tooling. */
export function isDev(cmdline, command) {
...
export function isStale(uptimeSeconds, cpu) {
...
export function parseSsListeners(stdout) {
    ...
    for (const raw of stdout.split('\n')) {
        ...
        for (const m of usersPart.matchAll(SS_USER_RE)) {
            ...
            const entry = procs.get(pid) ?? { command, sockets: new Set() };
            if (!entry.command)
...
export function addressFromSocket(socket) {
    const idx = socket.lastIndexOf(':');
