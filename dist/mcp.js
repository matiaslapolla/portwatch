...
export function buildServer() {
    ...
    server.registerTool('list_ports', {
        title: 'List listening ports',
        description: 'List processes listening on local TCP ports, enriched with the project/git-worktree that owns each one, docker container, staleness and optional health. Use this to see what is running before starting a dev server.',
        inputSchema: {
            dev_only: z
                .boolean()
                .optional()
    ...
    server.registerTool('whats_on_port', {
        title: 'What is on a port',
        description: 'Identify which process is listening on a specific port, including the project/worktree it belongs to. Returns an empty list if the port is free.',
        inputSchema: {
    ...
    server.registerTool('free_port', {
        title: 'Free a port',
        description: 'Reclaim a port by terminating its listeners (SIGTERM, then SIGKILL on holdouts). SAFETY: by default this refuses to kill non-dev-server processes (e.g. databases) — pass force=true to override. Prefer calling whats_on_port first.',
        inputSchema: {
        ...
        if (nonDev.length > 0 && !force) {
            return text({
                ...
                freed: false,
                refused: true,
                reason: 'Port is held by non-dev-server process(es). Re-run with force=true if you are sure.',
    ...
    server.registerTool('kill_process', {
        ...
        if (!force) {
            ...
            if (known && !known.isDev) {
                return text({
                    ...
                    killed: false,
                    refused: true,
                    reason: 'pid is a non-dev-server listener (e.g. a database). Re-run with force=true if you are sure.',
        ...
        try {
            ...
            return text({ pid, killed: false, error: err.message });
        }
    });
    server.registerTool('wait_for_port', {
        title: 'Wait for a port',
        description: 'Block until a port becomes free or starts listening, or until the timeout elapses. Useful after starting/stopping a server.',
        inputSchema: {
