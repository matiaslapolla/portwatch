...
export function parsePort(arg: string | undefined): number | undefined {
	if (!arg) return undefined;
	const port = Number(arg.replace(/^:/, ''));
	return Number.isInteger(port) && port > 0 && port < 65536 ? port : undefined;
}

function labelFor(proc: {project?: string; branch?: string; command: string}): string {
	if (proc.project && proc.branch) return `${proc.project} (${proc.branch})`;
	return proc.project ?? proc.command;
}

/** `portwatch free <port>` — terminate everything listening on a port. */
export async function cmdFree(arg: string | undefined): Promise<number> {
	...
	if (result.freed) {
		console.log(
			`Freed :${port} — ${how} → pid ${result.killed.join(', ')}.`,
		);
		return 0;
	}
...
export async function cmdKill(
	...
	if (Number.isInteger(pid) && pid > 0) {
		try {
			...
			console.error(`Failed to kill ${pid}: ${(err as Error).message}`);
			return 1;
		}
	}
...
export async function cmdWait(
	arg: string | undefined,
	opts: {target: WaitTarget; timeoutSec: number},
): Promise<number> {
	const port = parsePort(arg);
	if (port === undefined) {
 ...
 */
async function reportConfig(listening: Set<number>): Promise<void> {
	const cfg = await loadConfig();
	if (!cfg || cfg.expected.length === 0) return;
...
export async function cmdList(opts: {json?: boolean; health?: boolean} = {}): Promise<number> {
	...
	for (const row of rows) {
		const tags = [
			...
			.join(' ');
		console.log(
			`:${row.ports.join(',')}\t${row.pid}\t${labelFor(row)}\t${row.cmdline}${tags ? `\t[${tags}]` : ''}`,
		);
	}
